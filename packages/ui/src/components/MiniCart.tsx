        ...
        <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Cart">
          ...
          <aside className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col bg-white shadow-2xl">
            <header className="flex items-center justify-between border-b border-neutral-200 px-6 py-5">
              ...
              <button type="button" onClick={() => setOpen(false)} aria-label="Close" className="text-xl leading-none text-neutral-700 hover:text-neutral-900">
                ×
              </button>
            </header>

            {error ? (
              <p role="alert" className="mx-6 mt-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">
                {error}
              </p>
            ) : null}

            <div className={`flex-1 overflow-y-auto px-6 ${isPending ? "opacity-60" : ""}`}>
              {lines.length === 0 ? (
                <p className="py-16 text-center text-sm text-neutral-500">Your cart is empty.</p>
              ) : (
                lines.map((line) => (
                  <MiniCartRow
                    key={line.id}
            ...
            {lines.length > 0 ? (
              <footer className="border-t border-neutral-200 bg-neutral-100 px-6 pb-6 pt-4">
                <p className="mb-3 text-xs uppercase tracking-wide text-neutral-500">Summary</p>
                <dl className="flex flex-col gap-2 text-sm text-neutral-600">
                  ...
                  <div className="flex items-center justify-between">
                    <dt>Cash Total</dt>
                    <dd className="font-medium text-neutral-900">{formatMoney({ amount: String(cashTotal), currencyCode: currency })}</dd>
                  </div>
                  <div className="flex items-center justify-between">
                    <dt>Delivery</dt>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-neutral-300 pt-4">
                  <span className="text-sm text-neutral-600">Total due</span>
                  <span className="text-xl font-semibold text-neutral-900">{formatMoney({ amount: String(cashTotal), currencyCode: currency })}</span>
                </div>
                {overBalance ? (
                  <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    {pointsTotal.toLocaleString()} points selected but you have {points!.balance!.toLocaleString()}. Checkout won&apos;t
                    apply any points until you switch a line to dollars.
                  </p>
                ) : null}
                <a
                  href={href}
      ...
      <div className="flex gap-4">
        <div className="relative h-24 w-20 flex-shrink-0 overflow-hidden rounded-md border border-neutral-200 bg-neutral-100">
          {line.merchandise.image ? (
            <Image src={line.merchandise.image.url} alt={line.merchandise.image.altText ?? line.merchandise.product.title} fill sizes="80px" className="object-cover" />
          ) : (
            <span className="flex h-full items-center justify-center text-xs uppercase tracking-wide text-neutral-400">Pack</span>
          )}
        </div>
        <div className="flex flex-1 flex-col gap-1">
          ...
          {variantLabel ? <span className="text-xs uppercase tracking-wide text-neutral-500">{variantLabel}</span> : null}
          <div className="mt-2 flex items-center justify-between">
            <div className="inline-flex items-center rounded-full bg-neutral-100 text-sm">
              <button type="button" disabled={disabled || line.quantity <= 1} onClick={() => onQuantity(line.quantity - 1)} className="px-3 py-1.5 text-neutral-700 disabled:opacity-40" aria-label="Decrease quantity">
                −
              </button>
              <span className="min-w-[1.5rem] text-center text-neutral-900">{line.quantity}</span>
              <button type="button" disabled={disabled} onClick={() => onQuantity(line.quantity + 1)} className="px-3 py-1.5 text-neutral-700 disabled:opacity-40" aria-label="Increase quantity">
                +
              </button>
            </div>
            <button type="button" disabled={disabled} onClick={onRemove} aria-label="Remove" className="text-neutral-500 hover:text-neutral-900 disabled:opacity-40">
      ...
      {canToggle ? (
        <div className="mt-3 flex items-center justify-between rounded-md bg-neutral-100 px-3 py-2 text-sm">
          <span className="text-xs uppercase tracking-wide text-neutral-500">
            Paying with <span className="ml-1 text-sm font-semibold normal-case tracking-normal text-neutral-900">{usePoints ? "Points" : "Dollars"}</span>
          </span>
          <button type="button" disabled={disabled} onClick={onToggle} className="font-semibold text-neutral-900 underline underline-offset-2 disabled:opacity-40">
            Switch
          </button>
        </div>
      ) : null}
